...
- **能力代理统一出口**：数据获取经**能力代理服务端**（`packages/asgk-server`）出网——服务端持有全部上游知识（URL/编码/字段映射/签名/协议），按数据域暴露 21 个具名能力（`POST /v1/<capability>`），客户端只发语义请求（如「要 600519 实时行情」），**零上游知识**。客户端设 `ASGK_SERVER=http://127.0.0.1:7701` 指向服务端；未配/不可达时回退旧 `em_get` 路径（`ASGK_GW`/sgw，已 DEPRECATED）。
...
- **代码复用**：数据获取逻辑沉淀为共享脚本库（`skills/<name>/scripts/` + `packages/asgk-server`），agent 调用而非每次重新拼装脚本，降低 token 消耗与重复请求。
...
│   ├── asgk-server/     能力代理服务端（持有全部上游知识 + 流量内核，主出口）
│   └── sgw/             透明流量网关（DEPRECATED，保留作旧路径回退）
...
- `ref/a-stock-data`：A股全栈数据工具包（10 层 / 43 端点 / 15 数据源），作为 `skills/a-stock-data` 的**改造蓝本**。
  ...
  - 它是**单 agent 单文件**形态（127KB SKILL.md 全量入上下文），已按 `.agents/notes/capability-proxy-design.md` 重构为本项目形态（能力代理 + 拆分加载 + 代码沉淀）：`packages/asgk-server` 持有全部上游知识，`skills/a-stock-data` 客户端只发语义请求。
