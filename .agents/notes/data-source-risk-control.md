...
> **迁移状态更新（能力代理重构后）**：本文件复核的「直连源不安全」结论已被
> 能力代理架构**根治**——全部 21 个能力（含原标 `via="direct"` 的腾讯/百度/新浪/
> mootdx/legulegu）现在都经 `packages/asgk-server` 出网，服务端按源分组做令牌桶
> 限流（东财 ≤1 req/s、mootdx 2 req/s、legulegu 0.5 req/s 等）+ 熔断 + 缓存。
> 客户端不再直连任何源（除非服务端未部署回退 sgw，已 DEPRECATED）。
>
> 本文件的逐源风控结论（封 IP 阈值、协议栈画像、CSRF 等）**仍然有效**，作为
> 服务端 `config.toml` 各组 rps 取值的依据。
