...
| 档 | 含义 | 端点（数） | 代表 |
|----|------|-----------|------|
| **P 静态** | 发布即定稿，永不改 | 8 | 日K线、百度K线、个股/行业研报、研报PDF、分红历史、F10公司资料、巨潮公告 |
...
```toml
[cache]
# P 静态: 事件定稿型,永不变 → 长缓存(7天),TTL 到期才重拉
P_ttl = 604800            # 7天。研报/公告/分红/F10/日K历史
...
- **P（静态，7天）**：研报/公告/分红/互动易这类「事件追加型」——单条一经发布永久不变，但列表会增长。缓存 key 区分「取列表」（TTL 内可能漏新条目，可接受；7天后重拉）vs「取单条」（key 含记录id，命中即永久正确）。日K历史K线同理永久不变。**缓存价值最高**。
...
```
P(7d):  日K(1.1)·百度K线(1.3)·个股研报(2.1)·研报PDF(2.1)·行业研报(2.1)
        ·iwencai搜研报(2.3)·分红历史(4.4)·F10公司资料(6.2)·巨潮公告(7.1)·互动易(10.1)
...
R(no-cache): 分钟K(1.1)·五档盘口(1.1)·逐笔(1.1)·腾讯行情(1.2)·北向分钟(3.2)
             ·分钟资金流(3.4)·行业排名盘中(3.7)·[盘中]涨停/炸板/跌停池(8.1)
             ·涨停揭秘(8.2)·打板情绪(8.3)·期权T型报价(9.1b)·希腊字母(9.1c)
             ·热榜(10.2a)·人气榜(10.2b)
