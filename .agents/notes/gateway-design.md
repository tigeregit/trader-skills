...
#### 3.4.7 分档校准：用实测数据反推规则（可观测性驱动）

§3.4.6 的分流依赖人定的 P/L/S/R/N 规则，但人可能判错（日K线曾误判为静态就是例证：历史K线不变，但端点返回"最近N根"含今日实时根）。本节设计一个**用真实变更频率校准/修正分档**的机制。

**核心思路**：记录每次请求的响应指纹，事后统计"同一 key 的响应多久变一次"，用实测变更频率验证预设档位是否合理，发现矛盾就修正。
...
分析产出一份「分档修正建议表」，回填到 asgk 的 `@cache(tier=)` 标注和 `sgw_config.toml` 兜底规则。

**为何不做在线自适应（运行时自动调 TTL）**：
- **冷启动无数据**：新端点/新 key 初期无观测，自适应无依据。
- **抖动风险**：偶发的响应变化（如盘中瞬时波动）会触发 TTL 缩短，之后又延长，缓存行为不稳定。
- **动态字段误判**：剔除规则本身需要人来定，自适应无法自动处理 req_trace 这类陷阱。
- **定位**：分档是「数据性质」问题，性质相对稳定（龙虎榜永远盘后定稿），适合人+离线数据确认，不适合实时自调整。

**与 P4 的衔接**：离线校准作为 `skill-integration-test.md`（P4）的一部分——压测期间积累日志，事后分析产出修正表。MVP（P0）阶段先记录日志但不做分析（分析依赖足够样本，需 P4 的并发压测才积累得到）。
