...
class CmdSpec:
    ...
    category:   大类名（行情/基本面/研报/资金/信号/事件/风控/资讯/衍生）。
...
COMMANDS: list[CmdSpec] = [
    ...
    CmdSpec("研报", "eps", "datacenter", "一致预期EPS（ Thompson/THS）",
            data_type="table",
            fixed={"report_name": "RPT_WEB_RESPREDICT_HS",
                   "dc_source": "WEB"},
            args=[_code()]),
    CmdSpec("研报", "valuation", "quote", "完整估值快照（串联行情+EPS）",
            data_type="kv", args=[_code()]),
    # 3 个纯计算（local）
    CmdSpec("研报", "fwd_pe", "", "远期PE = 股价/一致预期EPS（纯计算）",
            data_type="kv", local=True, local_fn="forward_pe",
            args=[_num("price", "当前股价"),
                  _num("eps_forecast", "一致预期EPS")]),
    CmdSpec("研报", "digest", "", "PE消化年数（纯计算）",
    ...
    # ── 7. 风控（8）─────────────────────────────────────────────
    CmdSpec("风控", "pledge", "datacenter", "股权质押比例",
            data_type="table",
            fixed={"report_name": "RPT_PLEDGE_RATIO"},
            args=[_date()]),
    CmdSpec("风控", "goodwill", "datacenter", "商誉",
            data_type="table",
            fixed={"report_name": "RPT_GOODWILL"},
            args=[_date()]),
    CmdSpec("风控", "zt", "limitup_pool", "涨停池",
            data_type="table",
            fixed={"pool_type": "zt"},
            args=[_date()]),
    CmdSpec("风控", "zb", "limitup_pool", "炸板池",
            data_type="table",
            fixed={"pool_type": "zb"},
            args=[_date()]),
    CmdSpec("风控", "dt", "limitup_pool", "跌停池",
            data_type="table",
            fixed={"pool_type": "dt"},
            args=[_date()]),
    CmdSpec("风控", "yzt", "limitup_pool", "昨涨停池",
            data_type="table",
            fixed={"pool_type": "yzt"},
            args=[_date()]),
    CmdSpec("风控", "ths_zt", "ths_signal", "同花顺涨停池",
            data_type="table",
            fixed={"signal_type": "limit_up_pool"},
            args=[_date()]),
    CmdSpec("风控", "sentiment", "ths_signal", "打板情绪（炸板率等）",
            data_type="kv",
            fixed={"signal_type": "limit_up_sentiment"},
            args=[_date()]),

    # ── 8. 资讯（7）─────────────────────────────────────────────
    CmdSpec("资讯", "stock", "news", "个股新闻",
            data_type="table",
            fixed={"news_type": "stock"},
            args=[_code(), _opt("page_size", "每页条数", 20, type=int)]),
    CmdSpec("资讯", "telegraph", "cls_telegraph", "财联社电报",
            data_type="table",
            args=[_opt("page_size", "每页条数", 50, type=int)]),
    CmdSpec("资讯", "global", "news", "全球资讯",
            data_type="table",
            fixed={"news_type": "global"},
            args=[_opt("page_size", "每页条数", 50, type=int)]),
    CmdSpec("资讯", "hot_list", "ths_signal", "同花顺热榜",
            data_type="table",
            fixed={"signal_type": "hot_list"},
            args=[_opt("period", "周期(hour/day/week)", "hour")]),
    CmdSpec("资讯", "rank", "em_hot", "东财人气榜",
            data_type="table",
            fixed={"hot_type": "rank"},
            args=[_opt("top", "前N", 50, type=int)]),
    CmdSpec("资讯", "concept", "em_hot", "热门概念",
            data_type="table",
            fixed={"hot_type": "concept"},
            args=[_code()]),
    CmdSpec("资讯", "announce", "cninfo", "公告列表",
            data_type="table",
            fixed={"cninfo_type": "announce"},
            args=[_code(), _opt("page_size", "每页条数", 30, type=int)]),

    # ── 9. 衍生（5）─────────────────────────────────────────────
    CmdSpec("衍生", "opt_codes", "sina_option", "期权合约列表",
            data_type="kv",
            fixed={"option_type": "codes"},
            args=[_opt("underlying", "标的代码", "510050"),
                  _opt("call", "认购(True)/认沽(False)", True)]),
    CmdSpec("衍生", "opt_quote", "sina_option", "期权报价",
            data_type="kv",
            fixed={"option_type": "tquote"},
            args=[ArgSpec(name="code", desc="期权合约代码")]),
    CmdSpec("衍生", "opt_greek", "sina_option", "期权希腊字母",
            data_type="kv",
            fixed={"option_type": "greeks"},
            args=[ArgSpec(name="code", desc="期权合约代码")]),
    CmdSpec("衍生", "announce_pdf", "docs", "公告PDF原文下载",
            data_type="doc",
            fixed={"doc_type": "announce_pdf"},
            args=[ArgSpec(name="anno_id", desc="公告ID(annoId)"),
                  _code()]),
    CmdSpec("衍生", "report_pdf", "docs", "研报PDF原文下载",
            data_type="doc",
